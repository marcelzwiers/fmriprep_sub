...
def main(datadirs: list, maxwalltime_: float, maxmem_: float):
    ...
    for n, datadir in enumerate(datadirs):
        axs[n,0].hist(walltime[datadir], bins=100, range=(0,min(maxwalltime,maxwalltime_)))
        axs[n,1].hist(     mem[datadir], bins=100, range=(0,min(maxmem,maxmem_)))
...
if __name__ == '__main__':
    ...
    main(datadirs=[Path(datadir) for datadir in args.datafolders], maxwalltime_=args.walltime, maxmem_=args.mem)
