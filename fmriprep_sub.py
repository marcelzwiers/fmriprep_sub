...
def main(bidsdir: str, outputdir: str, workroot: str, subject_label=(), force=False, mem_mb=20000, walltime=72, file_gb_=50, nthreads=None, argstr='', qargstr='', dryrun=False, skip=True):
        ...
        if force or not report.is_file() or not all(sessions):
            ...
            job = textwrap.dedent('''\
                ...
                .format(pwd         = Path.cwd(),
                        ...
                        fmriprep    = f'apptainer run --cleanenv --bind \\$TMPDIR:/tmp,\\$TMPDIR:/var/tmp {os.getenv("DCCN_OPT_DIR")}/fmriprep/{version}/fmriprep-{version}.simg',
                        bidsdir     = bidsdir,
