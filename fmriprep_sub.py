        ...
        if force or not report.is_file() or not all(sessions):
            ...
            command = """qsub -l nodes=1:ppn={nthreads},walltime={walltime}:00:00,mem={mem_mb}mb{file_gb} -N fmriprep_sub-{sub_id} {qargs} <<EOF
                         ...
                         .format(pwd         = Path.cwd(),
                                 ...
                                 fmriprep    = f'unset PYTHONPATH; export PYTHONNOUSERSITE=1; singularity run --cleanenv --env TMP=\$TMPDIR,TMPDIR=\$TMPDIR,TEMP=\$TMPDIR,TEMPDIR=\$TMPDIR {os.getenv("DCCN_OPT_DIR")}/fmriprep/{version}/fmriprep-{version}.simg',
                                 bidsdir     = bidsdir,
